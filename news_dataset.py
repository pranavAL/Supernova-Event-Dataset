...
ANALYSIS_MODEL = "qwen2.5:14b"  #qwq, gemma3:27b Model used for personality analysis
VECTOR_STORE_DIR = "vector_stores"
RESULTS_DIR = "news_data_results"
OUTPUT_FILE = "news_events_analysis.json"
NEWS_EVENTS_DIR = "major-news-events"
...
def load_news_events(directory=NEWS_EVENTS_DIR, event_name=None):
    """Load news event text files."""
    ...
    logging.info(f"Successfully loaded {len(documents)} news event files")
...
def create_retriever(vector_db, llm):
    ...
    Your task is to extract exactly **5 critical developments or key occurrences** from the provided news event. Identify key moments, decisions, or turning points that significantly impacted how the event unfolded.

    Instructions:
    - Identify exactly 5 key developments or pivotal moments.
    - For each development, consider factors such as:
        - Did it significantly alter the course of the event?
        - Was it a turning point that led to different outcomes?
        - If this development had not occurred, would the overall outcome be drastically different?
        - Did it have long-term consequences on subsequent events or decisions?
    - Rank the 5 developments from **most critical to least critical** based on their cascading effects.
    - Provide a concise summary of each development in one or at most two sentences.

    Return a ranked list of the 5 critical developments with their summaries.
    """,
    )
...
def create_chain(retriever, llm):
    ...
    1. Identify exactly 5 critical developments or key occurrences from the news event.
    2. Rank these developments from **most critical to least critical** based on their impact or cascading effects.
    3. For each development, provide a concise summary in one or two sentences.
    4. Explain why the top-ranked development is the most critical, including:
    - How it had a cascading effect on the rest of the news event.
    - How the event would have unfolded differently if this development had not occurred.
    - Why this development stands out as the most critical.

    Your final answer should include:
    - A ranked list of exactly 5 critical developments with their summaries.
    - A clear statement of the most critical development and its impact.
    """
...
def extract_critical_events(response, model_name):
    ...
    if model_name == "phi4":
        ...
        explanation_pattern = r"(?:why this development|why this event|cascading effect|most critical).*?(?=\n\n|$)"
        ...
        ranked_list_section = ""
        ranked_pattern = r"(?:ranked list|the five critical|developments is|events is).*?(?:\d+\.\s+.*(?:\n|$))+"
        ...
        explanation_pattern = r"most critical development is.*?because:.*?(?=\n\n|$)"
        explanation_match = re.search(explanation_pattern, response, re.IGNORECASE | re.DOTALL)
        if explanation_match:
            top_event_explanation = explanation_match.group(0).strip()
        ...
        explanation_pattern = r"(?:why it\'s most critical|most critical development|most critical factor|cascading effect).*?(?=\n\n|$)"
        explanation_match = re.search(explanation_pattern, response, re.IGNORECASE | re.DOTALL)
        if explanation_match:
            top_event_explanation = explanation_match.group(0).strip()
    ...
    if critical_events:
        ...
        return ["No critical developments identified"], ""
...
def llm_determined_personality(full_response, critical_events, model_name, event_name):
    ...
    prompt = f"""You are evaluating the personality of an AI model based purely on the **themes and focus** of the critical events it selects from a news story. **Do not** classify based on reasoning style (e.g., "Logical", "Analytical", "Methodical"). Instead, focus only on the nature of the events themselves.
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(personality)
            # personality = personality.split("</think>")[1].strip()
            break
        except Exception as e:
...
def analyze_news_event(event_docs, event_title):
    """Analyze a news event."""
    logging.info(f"Analyzing news event: {event_title}")
    ...
    for model_name in MODELS:
        ...
        try:
            ...
            # Define the question
            question = f"Identify exactly 5 critical developments or key occurrences from the news event: {event_title}?"
            ...
            if not critical_events:
                critical_events = ["No critical developments identified"]
...
def analyze_all_news_events():
    """Analyze all news events in the directory."""
    ensure_directories()
    
    # Load all news events
    documents, event_titles = load_news_events()
    
    if not documents:
        logging.error("No news event documents to analyze")
        return None
    ...
    for i, (doc, event_title) in enumerate(zip(documents, event_titles)):
        ...
        event_result = analyze_news_event([doc], event_title)
...
def analyze_specific_news_event(event_name):
    """Analyze a specific news event by name."""
    ensure_directories()
    
    # Load the specific news event
    documents, event_titles = load_news_events(event_name=event_name)
    
    if not documents:
        logging.error(f"News event '{event_name}' not found or failed to load")
        return None
    
    # Analyze the event
    event_result = analyze_news_event(documents, event_titles[0])
...
def generate_reports():
    ...
    # Save to CSV
    df.to_csv(os.path.join(RESULTS_DIR, "news_events_analysis_summary.csv"), index=False)
    
    # Generate model personality statistics
    personality_stats = df.groupby(['Model', 'Model_Personality']).size().reset_index(name='Count')
    personality_stats.to_csv(os.path.join(RESULTS_DIR, "news_model_personality_stats.csv"), index=False)
...
def generate_html_report(df):
    ...
    <html>
    <head>
        <title>News Events Critical Analysis</title>
        <style>
    ...
    <body>
        <h1>News Events Critical Analysis</h1>
        <p>Analyzed {event_count} news events using {model_count} different models with LLM-based personality detection.</p>
    ...
    html_content += """
        ...
        <h2>Sample News Event Analysis</h2>
    """
    ...
    for event in df['Event'].unique()[:5]:  # Show first 5 events
        ...
        for _, row in event_data.iterrows():
            ...
            <div class="model-card">
                ...
                <p><strong>Top Critical Development:</strong> {row['Top_Critical_Event']}</p>
                <p><strong>Why Critical:</strong> {row['Why_Critical']}</p>
                <p><strong>Other Key Developments:</strong></p>
                <ol start="2">
            """
    ...
    with open(os.path.join(RESULTS_DIR, "news_events_report.html"), "w") as f:
...
def create_model_personality_table():
    ...
    try:
        ...
        # Save to CSV
        comparison_df.to_csv(os.path.join(RESULTS_DIR, "news_model_personality_comparison.csv"), index=False)
        ...
        <html>
        <head>
            <title>Model Personality Comparison for News Events</title>
            <style>
                body {{ font-family: Arial, sans-serif; margin: 20px; }}
                h1 {{ color: #333; }}
                table {{ border-collapse: collapse; width: 100%; margin-bottom: 20px; }}
        ...
        <body>
            <h1>Model Personality Comparison Across News Events</h1>
        ...
        # Save the HTML table
        with open(os.path.join(RESULTS_DIR, "news_model_personality_comparison.html"), "w") as f:
            f.write(html_table)
...
def main():
    ...
    # Example usage - analyze all news events
    all_results = analyze_all_news_events()
